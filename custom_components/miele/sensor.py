...
def state_capability(type, state):
    ...
    capabilities = {
        ...
        "7": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "signalInfo",
              "signalFailure", "remoteEnable", "elapsedTime", "ecoFeedback"],
        "12": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "13": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "14": ["status", "signalFailure", "plateStep"],
        "15": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "16": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "17": ["ProgramID", "status", "programPhase", "signalInfo", "signalFailure", "remoteEnable"],
        "18": ["status", "signalInfo", "signalFailure", "remoteEnable", "ventilationStep"],
        "19": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "signalDoor",
               "remoteEnable"],
        "20": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "signalDoor",
               "remoteEnable"],
        "21": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "signalDoor",
               "remoteEnable"],
        "23": ["ProgramID", "status", "programType", "signalInfo", "signalFailure", "remoteEnable", "batteryLevel"],
        "24": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "signalInfo",
               "signalFailure", "remoteEnable", "elapsedTime", "spinningSpeed", "dryingStep", "ecoFeedback"],
        "25": ["status", "signalInfo", "signalFailure"],
        "27": ["status", "signalFailure", "plateStep"],
        "31": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "32": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "signalDoor",
               "remoteEnable"],
        "33": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "signalDoor",
               "remoteEnable"],
        "34": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "signalDoor",
               "remoteEnable"],
        "45": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "67": ["ProgramID", "status", "programType", "programPhase", "remainingTime", "startTime", "targetTemperature",
               "temperature", "signalInfo", "signalFailure", "signalDoor", "remoteEnable", "elapsedTime"],
        "68": ["status", "targetTemperature", "temperature", "signalInfo", "signalFailure", "remoteEnable"]
    }
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for k, device in devices.items():
        ...
        sensors = []
        if "status" in device_state and state_capability(type=device_type,state="status"):
            sensors.append(MieleStatusSensor(hass, device, "status"))

        if "ProgramID" in device_state and state_capability(type=device_type,state="ProgramID"):
            sensors.append(MieleTextSensor(hass, device, "ProgramID"))

        if "targetTemperature" in device_state and state_capability(type=device_type,state="targetTemperature"):
            for i, val in enumerate(device_state["targetTemperature"]):
                sensors.append(
                    MieleTemperatureSensor(hass, device, "targetTemperature", i)
                )
        if "temperature" in device_state and state_capability(type=device_type,state="temperature"):
            for i, val in enumerate(device_state["temperature"]):
                sensors.append(MieleTemperatureSensor(hass, device, "temperature", i))

        if "remainingTime" in device_state and state_capability(type=device_type,state="remainingTime"):
            sensors.append(MieleTimeSensor(hass, device, "remainingTime"))
        if "startTime" in device_state and state_capability(type=device_type,state="startTime"):
            sensors.append(MieleTimeSensor(hass, device, "startTime"))
        if "elapsedTime" in device_state and state_capability(type=device_type,state="elapsedTime"):
